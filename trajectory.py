...
class atmosphere:
    ...
    def __init__(self):
        grab_levels = [1000,  975,  950,  925,  900, 850,  800,  750,  700,  650,  600, 550, 500, 450, 400, 350, 300, 250, 200, 150, 100, 70, 50, 30, 20, 10, 7, 5, 3, 2, 1, "surface"]
        now = datetime.now()
        hr = 0
        date_hr = "%d%02d%02d%02d" % (now.year, now.month, now.day, hr)        
        
        variables = ["TMP", "UGRD", "VGRD", "HGT", "RH"]
#        self.download_file( (-10,50), (250,350), variables, grab_levels, date_hr, "file.anl")         
    ...
    def get_P_interp(self, i, j, k):
        """"
        return a function that uses linear interpolation to predict pressure value near grid point (i,j,k)
